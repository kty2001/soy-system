...
import { processDenoise } from '../utils/api';
...
const DenoisePage = () => {
  ...
  const handleFileDrop = async (file) => {
    ...
    try {
      const response = await processDenoise(file);
      setResult(response);
    } catch (err) {
  ...
  return (
    <div className="p-lg">
      <h1 className="text-textPrimary text-3xl mb-md">이미지 디노이징</h1>
      <p className="text-textSecondary mb-xl max-w-[800px] leading-relaxed">
        THz 이미지의 노이즈를 제거하여 이미지 품질을 향상시킵니다.
        이미지 파일을 업로드하면 AI 모델이 이미지를 처리하여 노이즈가 제거된 이미지를 생성합니다.
      </p>
      ...
      {loading ? (
        ...
        <ResultDisplay result={result} metricName="노이즈 레벨" />
      )}
    </div>
  );
};
